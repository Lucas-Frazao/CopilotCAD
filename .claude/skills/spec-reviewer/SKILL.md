...
name: spec-reviewer
description: >-
  Reviews a CopilotCAD F-0xx spec against docs/feature_roadmap.md and
  docs/architecture.md for conflicts, missing acceptance criteria, and scope
...
- Spec path: `specs/F-<id>__<name>.md`

## Review checklist

1. **Roadmap alignment** — Requirements match `docs/feature_roadmap.md` section for this F-id.
2. **Architecture alignment** — No contradiction with `docs/architecture.md` (IPC, kernel adapter, file ownership).
...
6. **Data model** — Contracts match existing schemas (`specs/F-001_*`, F-002 workspace manifest, etc.).
...
2. Read the F-0xx section in `docs/feature_roadmap.md`.
3. Read relevant sections in `docs/architecture.md`.
...
"Compare specs/F-0xx_*.md acceptance criteria against changed files in the branch."
